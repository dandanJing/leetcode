...
int malcin(){
  ...
  FILE * fp_out=freopen("output1.txt","w",stdout);
  int te;
  cin >> te;
  FOR(nr, 1, te){
    printf("Case #%d: ", nr);
    ...
    REP(a, n) if (a != pocz){
      ...
      REP(i, n) REP(j, n)
        if (graf[i][j] < INF && dist[pocz][i] + graf[i][j] + dist[j][a] == dist[pocz][a])
			for(vector<int>::iterator it=nums[i][j].begin();it!= nums[i][j].end();it++){
            if(mian) wyn[*it] += (LD)cnt_sc[pocz][i] * (LD)cnt_sc[j][a] / (LD)mian;
          }
    }
    REP(i, m) if(dupa) wyn[i] /= (LD)dupa;
